...
## Usage
Upload the plugin folder to `/wp-content/plugins/`, activate it through the Plugins menu, and a **Sweep** menu appears in the sidebar. Every sweep is irreversible, so back your database up first.

The Sweep screen lists every sweep with how many items it would remove and what proportion of that table they are. Tick the ones you want and use the **Sweep** bulk action, or use the Sweep and Details row actions one at a time. Nothing on the screen needs JavaScript: the row actions are ordinary links and the bulk action is an ordinary form.

**Sweep → Settings** carries one setting, how many items the Details view lists. The default is 500, and the cap exists because the whole sample is held in memory and written into the page.
...
* `wp_sweep_limit_details` (int) — how many items a Details list shows, overriding the setting.
* `wp_sweep_capability` (string `$capability`, string `$context`) — the capability required. `$context` is one of `sweep`, `settings`, `ajax` or `rest`.
...
### Where did the Tools -> Sweep screen go?

It has a menu of its own now. Look for **Sweep** in the sidebar rather than under Tools, and update any bookmark: the address changed from `tools.php?page=wp-sweep/admin.php` to `admin.php?page=wp-sweep`.
...
Two option rows, `wp_sweep_options` and `wp_sweep_version`, both of which `uninstall.php` removes — on every site of a network, not just the first hundred. It creates no database tables, registers no capabilities and schedules no events.

## Screenshots
1. The Sweep screen, listing every sweep with what it would remove
2. The Sweep screen after a bulk sweep
3. Sweep -> Settings

## Changelog
### 2.0.0
* BREAKING: Requires WordPress 6.8 and PHP 8.2, up from 6.0 and 7.4. A site on an older stack will not be offered the update.
* BREAKING: The screen moved out of Tools into a `Sweep` menu of its own, from `tools.php?page=wp-sweep/admin.php` to `admin.php?page=wp-sweep`.
...
* BREAKING: `WP_Sweep::$limit_details` is gone. Read it with `limit_details()`, change it in Sweep -> Settings, or filter `wp_sweep_limit_details`.
...
* NEW: A `Sweep -> Settings` screen built on the Settings API, holding the Details limit.
...
* NEW: `wp_sweep_capability` and `wp_sweep_limit_details` filters.
* NEW: Two option rows, `wp_sweep_options` and `wp_sweep_version`, and an `uninstall.php` that removes them across a network.
